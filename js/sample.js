$('#get-email').on('submit', function(event) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data === "1") {
                ...
                $('#get-password').find('input[name="password"]').focus();
            } else if (data === "2") {
...
$('#get-password').find('input[name="show-password"]').on('change', function() {
    if ($(this).prop('checked')) {
        $('#get-password').find('input[name="password"]').attr('type', 'text');
    } else {
        $('#get-password').find('input[name="password"]').attr('type', 'password');
    }
});
$('#sign-in-show-password-button').on('click', function() {
    $('#get-password').find('input[name="show-password"]').click();
})
$('#get-password').on('submit', function(event) {
    event.preventDefault();
    ...
    if (password.length < 8 || password.length > 16) {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                email: $('#get-email').find('input[name="email"]').val(),
...
$('#forgot-password').on('click', function () {
    var email = $('#get-email').find('input[name="email"]').val();
...
$('#verify-forgot-password').on('submit', function(event) {
    ...
    if (verification === otp) {
        $('#signin-3').hide();
        $('#signin-4').fadeIn('slow', function() {
            alert('You can update your password now if you forgot it. Requirements: 8-16 Digit, Contain atleast one alphabet and numeric.');
        });
        ...
        success: function (data) {
            ...
            if (data === "1") {
                ...
                $('#register-2').fadeIn('slow', function() {
                    alert('Requirements: 8-16 Digit, Contain atleast one alphabet and numeric.');
                });
