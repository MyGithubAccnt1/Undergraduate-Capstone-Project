...
$checksql = "SELECT COUNT(*) AS total_rows FROM product WHERE category = 'Directory'";
...
if ($filter === "None") {
	...
	$sql = "SELECT * FROM product WHERE category = 'Directory' ORDER BY popularity DESC LIMIT 8 OFFSET $page";
	...
	$sql = "SELECT * FROM product WHERE category = 'Directory' and price between '$min_price' and '$max_price' ORDER BY popularity DESC LIMIT 8 OFFSET $page";
	$result = $conn->query($sql);
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			echo '<div class="col-sm-12 col-md-4 col-lg-3 mb-3">';
