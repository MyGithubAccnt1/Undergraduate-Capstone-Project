...
if (mysqli_num_rows($result) > 0) {
    ...
    echo '
    <form id="change_product" class="container p-3">
        <input type="hidden" value="' . $row['id'] . '" name="id">
        <p class="mb-3"><small>Edit Product Details</small></p>
        <p class="mb-2">
            <small>Product Name: <input type="text" name="title" value="'. $row['title'] .'" required></small>
        </p>
        <p class="mb-2" style="display: flex; flex-direction: column; align-items: start; gap: 3px;">
            <small>Product Description: </small>
            <small><textarea rows="5" style="resize: none;" type="text" name="description" required>'. $row['description'] .'</textarea></small>
        </p>
        <p class="mb-2">
